...
from utils.eval import evaluate_list
...
class Seq2Seq(nn.Module):
    ...
    def __init__(self, encoder, raw_encoder, decoder, config, beam_size=None, max_length=None, sos_id=None, eos_id=None, l2_norm=False, fusion=True, layer_attention=False):
    ...
    def attention_encoder(self, cls_embeddings):
        alpha = F.softmax(torch.matmul(cls_embeddings, self.w), dim=1).unsqueeze(-1)
        # print(alpha[0, :, 0, 0])
        # print('-----')
        out = cls_embeddings * alpha
        out = torch.sum(out, 1)
        return out
    ...
    def forward(self, source_ids=None, source_mask=None, similarity_ids=None, similarity_mask=None, target_ids=None, target_mask=None):
        source_outputs = self.raw_encoder(source_ids, attention_mask=source_mask, return_dict=True, output_attentions=True, output_hidden_states=True)
        if(self.layer_attention==True):
            hidden_states = source_outputs.hidden_states
            cls_embeddings = hidden_states[1].unsqueeze(1)
            for i in range(2, 13):
                cls_embeddings = torch.cat((cls_embeddings, hidden_states[i].unsqueeze(1)), dim=1)
            source_outputs = self.attention_encoder(cls_embeddings)
        else:
            source_outputs = source_outputs.last_hidden_state
        # source_outputs [batch_size, seq_len, d_model]
        source_encoder_output = source_outputs.permute([1, 0, 2]).contiguous()
        # source_encoder_output [seq_len, batch_size, d_model]
        similarity_outputs = self.encoder(similarity_ids, attention_mask=similarity_mask, return_dict=True, output_attentions=True, output_hidden_states=True)
        if (self.layer_attention == True):
            hidden_states = similarity_outputs.hidden_states
            cls_embeddings = hidden_states[1].unsqueeze(1)
            for i in range(2, 13):
                cls_embeddings = torch.cat((cls_embeddings, hidden_states[i].unsqueeze(1)), dim=1)
            similarity_outputs = self.attention_encoder(cls_embeddings)
        else:
            similarity_outputs = similarity_outputs.last_hidden_state
        similarity_encoder_output = similarity_outputs.permute([1, 0, 2]).contiguous()
        raw_source_outputs = source_encoder_output
        ...
        encoder_output = raw_source_outputs+encoder_output
...
class CodeBert_Seq2Seq():
    def __init__(self, ip_path, raw_path, decoder_layers, fix_encoder, beam_size, max_source_length, max_target_length, load_model_path, layer_attention,l2_norm, fusion):
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        config_class, model_class, tokenizer_class = RobertaConfig, RobertaModel, RobertaTokenizer
        config = config_class.from_pretrained(ip_path)
        self.tokenizer = tokenizer_class.from_pretrained(ip_path)
        # length config
        self.max_source_length, self.max_target_length = max_source_length, max_target_length
        self.beam_size = beam_size
        self.fusion = fusion
        # build model
        encoder = model_class.from_pretrained(ip_path, attn_implementation="eager")
        raw_encoder = model_class.from_pretrained(raw_path, attn_implementation="eager")
        decoder_layer = nn.TransformerDecoderLayer(d_model=config.hidden_size, nhead=config.num_attention_heads)
        decoder = nn.TransformerDecoder(decoder_layer, num_layers=decoder_layers)
        self.model = Seq2Seq(encoder=encoder, raw_encoder=raw_encoder, decoder=decoder, config=config,
                        beam_size=beam_size, max_length=max_target_length,
                        sos_id=self.tokenizer.cls_token_id, eos_id=self.tokenizer.sep_token_id, l2_norm=l2_norm, fusion=fusion, layer_attention = layer_attention)
        ...
        if fix_encoder:
            ...
            raw_encoder.requires_grad_(False)
            decoder.requires_grad_(False).requires_grad_(False)
            print("冻结Codebert的参数")
        ...
        for epoch in range(num_train_epochs):
            ...
            if do_eval==True:
                ...
                for hyp, gold in zip(p, eval_examples):
                    csv_ref_list.append(gold.target)
                    csv_hyp_list.append(hyp)

                df = pd.DataFrame(csv_hyp_list)
                df.to_csv(os.path.join(output_dir, "valid_hyp.csv"), index=False, header=None)

                df = pd.DataFrame(csv_ref_list)
                df.to_csv(os.path.join(output_dir, "valid_ref.csv"), index=False, header=None)

                metrics_dict = evaluate_list(csv_ref_list, csv_hyp_list)
                print(metrics_dict)
                dev_bleu = round(metrics_dict['rouge-w'], 4)
                print("  %s = %s " % ("ROUGE_W", str(dev_bleu)))
                print("  " + "*" * 20)
                if dev_bleu > best_bleu:
                    count = 0
                    print("  Best ROUGE_W:%s", dev_bleu)
                    print("  " + "*" * 20)
                    best_bleu = dev_bleu
                    # Save best checkpoint for best bleu
                    output_dir_bleu = os.path.join(output_dir, 'checkpoint-best-rouge')
    ...
    def predict(self, source, similarity):
        # Pad/truncate đúng max_length để hai encoder có cùng chiều dài
